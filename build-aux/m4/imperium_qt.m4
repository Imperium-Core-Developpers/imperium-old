dnl Copyright (c) 2013-2016 The Imperium Core developers
dnl Distributed under the MIT software license, see the accompanying
dnl file COPYING or http://www.opensource.org/licenses/mit-license.php.

dnl Helper for cases where a qt dependency is not met.
dnl Output: If qt version is auto, set imperium_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$imperium_qt_want_version" = xauto && test "x$imperium_qt_force" != xyes; then
    if test "x$imperium_enable_qt" != xno; then
      AC_MSG_WARN([$1; imperium-qt frontend will not be built])
    fi
    imperium_enable_qt=no
    imperium_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "x$imperium_enable_qt" != xno && test "x$imperium_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build imperium-qt GUI (default=auto)])],
    [
     imperium_qt_want_version=$withval
     if test "x$imperium_qt_want_version" = xyes; then
       imperium_qt_force=yes
       imperium_qt_want_version=auto
     fi
    ],
    [imperium_qt_want_version=auto])
...
dnl Outputs: imperium_enable_qt, imperium_enable_qt_dbus, imperium_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "x$imperium_cv_static_qt" = xyes; then
    _BITCOIN_QT_FIND_STATIC_PLUGINS
    AC_DEFINE(QT_STATICPLUGIN, 1, [Define this symbol if qt plugins are static])
    AC_CACHE_CHECK(for Qt < 5.4, imperium_cv_need_acc_widget,[
      ...
      [imperium_cv_need_acc_widget=yes],
      [imperium_cv_need_acc_widget=no])
    ])
    if test "x$imperium_cv_need_acc_widget" = xyes; then
  ...
  BITCOIN_QT_CHECK([
    imperium_enable_qt=yes
    imperium_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      imperium_enable_qt_test=no
    fi
    imperium_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      imperium_enable_qt_dbus=yes
    fi
  ...
  AC_MSG_RESULT([$imperium_enable_qt (Qt5)])
...
dnl Output: imperium_cv_qt5=yes|no
AC_DEFUN([_BITCOIN_QT_CHECK_QT5],[
  AC_CACHE_CHECK(for Qt 5, imperium_cv_qt5,[
  AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
      #include <QtCore/qconfig.h>
      #ifndef QT_VERSION
      #  include <QtCore/qglobal.h>
...
AC_DEFUN([_BITCOIN_QT_CHECK_QT58],[
  AC_CACHE_CHECK(for > Qt 5.7, imperium_cv_qt58,[
  AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
      #include <QtCore/qconfig.h>
      #ifndef QT_VERSION
      #  include <QtCore/qglobal.h>
...
dnl Output: imperium_cv_static_qt=yes|no
dnl Output: Defines QT_STATICPLUGIN if plugins are static.
AC_DEFUN([_BITCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, imperium_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
      ...
      [imperium_cv_static_qt=yes],
      [imperium_cv_static_qt=no])
    ])
  if test "x$imperium_cv_static_qt" = xyes; then
...
AC_DEFUN([_BITCOIN_QT_FIND_STATIC_PLUGINS],[
    if test "x$qt_plugin_path" != x; then
     ...
     m4_ifdef([PKG_CHECK_MODULES],[
       if test x$imperium_cv_qt58 = xno; then
       ...
       if test "x$TARGET_OS" = xwindows; then
         AC_CACHE_CHECK(for Qt >= 5.6, imperium_cv_need_platformsupport,[
           AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
               #include <QtCore/qconfig.h>
               #ifndef QT_VERSION
               #  include <QtCore/qglobal.h>
           ...
           [imperium_cv_need_platformsupport=yes],
           [imperium_cv_need_platformsupport=no])
         ])
         if test "x$imperium_cv_need_platformsupport" = xyes; then
           if test x$imperium_cv_qt58 = xno; then
...
dnl Inputs: imperium_qt_want_version (from --with-gui=). The version to check
dnl         first.
dnl Inputs: $1: If imperium_qt_want_version is "auto", check for this version
dnl         first.
...
dnl Inputs: imperium_qt_want_version (from --with-gui=). The version to use.
...
AC_DEFUN([_BITCOIN_QT_FIND_LIBS_WITHOUT_PKGCONFIG],[
  ...
  BITCOIN_QT_CHECK([
    if test "x$imperium_qt_want_version" = xauto; then
