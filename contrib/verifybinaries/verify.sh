#!/usr/bin/env bash
# Copyright (c) 2016 The Imperium Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   imperiumcore.org and imperium.org and compares them.
...
WORKINGDIR="/tmp/imperium_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://imperiumcore.org"
HOST2="https://imperium.org"
BASEDIR="/bin/"
VERSIONPREFIX="imperium-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'imperium-',
...
if [ $? -ne 0 ]; then
   echo "imperium.org failed to provide signature file, but imperiumcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "imperium.org and imperiumcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Imperium Core binary release signing key installed?"
   fi
