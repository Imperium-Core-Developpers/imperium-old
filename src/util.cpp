...
const char * const BITCOIN_CONF_FILENAME = "imperium.conf";
const char * const BITCOIN_PID_FILENAME = "imperiumd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "imperiumd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Imperium
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Imperium
    // Mac: ~/Library/Application Support/Imperium
    // Unix: ~/.imperium
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Imperium";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Imperium";
#else
    // Unix
    return pathRet / ".imperium";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Imperium Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Imperium Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Imperium Core developers";
    }
