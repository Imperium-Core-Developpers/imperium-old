#!/usr/bin/env python3
# Copyright (c) 2015-2018 The Imperium Core developers
...
from test_framework.test_framework import ImperiumTestFramework
...
class FullBlockTest(ImperiumTestFramework):
    ...
    def run_test(self):
        ...
        # Extend the b26 chain to make sure imperiumd isn't accepting b26
        ...
        # Extend the b28 chain to make sure imperiumd isn't accepting b28
        ...
        # Create a transaction that spends one vote to the p2sh_script, the rest to OP_TRUE
        ...
        # Until block is full, add tx's with 1 vote to p2sh_script, the rest to OP_TRUE
        tx_new = None
        tx_last = tx
        ...
        # imperiumd doesn't disconnect us for sending a bloated block, but if we subsequently
        ...
        #                                                                                    \-> b68 (20)
        #
        # b68 - coinbase with an extra 10 votes,
        #       creates a tx that has 9 votes from out[20] go to fees
        #       this fails because the coinbase is trying to claim 1 vote too much in fees
        #
        # b69 - coinbase with extra 10 votes, and a tx that gives a 10 vote fee
        #       this succeeds
        #
        ...
        #    Low-S and High-S values (whereas the imperium code has custom code which does so);
    ...
    def next_block(self, number, spend=None, additional_coinbase_value=0, script=CScript([OP_TRUE]), solve=True):
        ...
        if spend is None:
            ...
            coinbase.vout[0].nValue += spend.vout[0].nValue - 1  # all but one vote to fees
            coinbase.rehash()
            block = create_block(base_block_hash, coinbase, block_time)
            tx = self.create_tx(spend, 0, 1, script)  # spend 1 vote
            self.sign_tx(tx, spend)
