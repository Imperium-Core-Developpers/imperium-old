...
from test_framework.util import assert_equal, assert_raises_rpc_error, vote_round, sync_blocks, sync_mempools

MAX_ANCESTORS = 25
MAX_DESCENDANTS = 25

class MempoolPackagesTest(ImperiumTestFramework):
    ...
    def run_test(self):
        ...
        for x in reversed(chain):
            descendant_fees += mempool[x]['fee']
            if (x == chain[-1]):
                assert_equal(mempool[x]['modifiedfee'], mempool[x]['fee']+vote_round(0.00002))
                assert_equal(mempool[x]['fees']['modified'], mempool[x]['fee']+vote_round(0.00002))
            assert_equal(mempool[x]['descendantfees'], descendant_fees * COIN + 2000)
            assert_equal(mempool[x]['fees']['descendant'], descendant_fees+vote_round(0.00002))
